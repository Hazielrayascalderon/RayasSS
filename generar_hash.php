...
// Permite el acceso libre solo si todavía no existe ningún usuario en el sistema
// (configuración inicial). En cuanto exista al menos un usuario, exige sesión
// de Administrador o Coordinación para poder generar hashes.
$hay_usuarios = true;
try {
    $db = new Conexion();
    $conn = $db->getConexion();
    $total = $conn->query("SELECT COUNT(*) FROM usuarios")->fetchColumn();
    $hay_usuarios = intval($total) > 0;
} catch (Exception $e) {
    // Si la tabla no existe todavía, tratamos esto como "sin usuarios" (primer uso)
    $hay_usuarios = false;
}
